...
// in .github/actions/breaking-change-footers/action.yml, shared across the estate.
//
...
const WORKFLOW = path.join(__dirname, '..', '.github', 'actions', 'breaking-change-footers', 'action.yml');
...
function extractRunBlock(yaml, key) {
    ...
    // The guard lives in a composite ACTION now, not a workflow job, so there is
    // exactly one `run: |` block and no job key to find. The test still EXTRACTS
    // it rather than carrying a copy: a test with its own copy of the script
    // passes while the real one rots.
    const start = 0;
    if (!/^runs:/m.test(lines.join('\n'))) return { error: `no \`runs:\` in ${path.relative(process.cwd(), WORKFLOW)}` };
