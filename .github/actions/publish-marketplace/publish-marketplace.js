...
//
// The implementation behind the `publish-marketplace` composite action
// (../publish-marketplace/action.yml), called identically by
// sethbacon/azure-pipelines-terraform, sethbacon/azure-pipelines-packer and
// sethbacon/azure-pipelines-release-docs. All three publish the same kind of
// artefact the same way, with a token minted from the same service principal.
//
// THIS USED TO BE THREE BYTE-IDENTICAL COPIES, one per consumer, kept in sync
// by hand. That is exactly how a real regression shipped: the ALREADY_PUBLISHED
// detector below false-matched tfx's own "Checking if this extension is already
// published" preamble (present on every attempt, not just failures), so a
// genuine Gallery API timeout on azure-pipelines-terraform's v1.15.2 release was
// misreported as a duplicate-version rejection and never retried, leaving the
// GitHub Release stuck in draft. The same defect sat unfired in the other two
// copies. This estate had already acquired three hand-copies of one HTTP client
// and an egress fix that reached only some of them before this file existed --
// the fix for THIS bug was the last one propagated by hand, and the reason it
// moved here rather than staying a fourth thing to remember to copy. One
// definition; a fix lands in every consumer the next time each one bumps its
// pinned SHA. Issue references below predate the move and are fully qualified
// for the same reason a bare `#109` used to resolve to a different issue in
// each repository.
//
...
//
// Invoked by ../publish-marketplace/action.yml via `github.action_path`, never
// called directly by a consumer. Usage:
//   node publish-marketplace.js --vsix <file> [--tfx <path>]
