...
//
// It fired THREE times in sethbacon/terraform-state-manager-backend before this
// guard existed, and the third was found only by pointing the guard at the right
// universe. The three are the evidence this file is built on, and they are cited
// by repository and number rather than as "here" because this module is shared:
// it runs in repositories where the defect has not fired YET, which is the point
// of adopting it.
//
...
//
// CORRECTION CARRIED FORWARD, re-measured 2026-08-30 on all ten estate
// repositories. An earlier version of this header said the body becomes the
// merge commit message via `squash_merge_commit_message=COMMIT_MESSAGES`, so the
// body could close issues through the commit-message path as well. IT DOES NOT.
// COMMIT_MESSAGES means the BRANCH's commit messages, and a release-please
// branch carries exactly one commit. The real merge commit of #480 is
//
//     chore(main): release 3.13.0 (#480)
//     Co-authored-by: ...
//
// -- no changelog, no compare link, no closing keyword. Both #245 and #459 were
// closed by the LINK GRAPH alone. Every repository in this estate is on
// COMMIT_MESSAGES today, so the commit-message path is not live in any of them.
//
// The body scan is still KEPT, as a clearly-labelled SECONDARY signal, but for
// the honest reason rather than the wrong one:
//
//   * the body is what GitHub PARSES to build the link graph, so a keyword in it
//     is a direct prediction of what the graph will hold, available at moments
//     the graph itself may not yet have been recomputed;
//   * a consumer that sets `squash_merge_commit_message=PR_BODY` DOES revive the
//     commit-message path, and this module is shared across repositories whose
//     settings this file cannot see. The union is never narrower than the
//     authoritative set, so keeping it costs nothing and covers that case.
//
// The graded universe is therefore the UNION of the two. Note which way round
// they go: a guard aimed at the merge commit message ALONE would grade an empty
// universe and report clean on both known incidents.
//
...
async function main() {
  ...
  summarise([
    ...
    '',
    'Merging this pull request closes every issue in GitHub\'s **linked-issue graph** for',
    'it. release-please puts issues in that graph by rendering every issue reference a',
    'commit carries as `closes [#N]` in the changelog -- including a deliberately',
    'non-closing `Refs #N` trailer -- and GitHub parses that body into the graph. Issues',
    'also reach the graph through the **Development panel**, which writes nothing into',
    'the body at all.',
    '',
    ...failed.map((r) =>
