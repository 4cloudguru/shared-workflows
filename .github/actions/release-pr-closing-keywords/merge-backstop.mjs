...
//
// PROVEN ON A REAL INCIDENT, in sethbacon/terraform-state-manager-backend --
// named by repository because this module is shared, and runs in repositories
// where the defect has not fired yet. Release pull request #243:
//
...
// commit message. Every repository in this estate sets
// `squash_merge_commit_message=COMMIT_MESSAGES` -- re-measured across all ten on
// 2026-08-30 -- which means the branch's commit messages, NOT the pull request
// body, and a release-please branch carries exactly one commit,
...
async function main() {
  ...
  // The branch prefix is an INPUT, not a literal. release-please's default is
  // `release-please--branches--`, but a consumer can change it in its manifest
  // config, and a guard that silently matched nothing would report "nothing to
  // check" on every release forever -- a clean-looking result over an empty
  // universe, which is the failure this whole guard exists to remove.
  const prefix = process.env.RELEASE_BRANCH_PREFIX || 'release-please--branches--';
  if (!pr.head.ref.startsWith(prefix)) {
    line(`Pull request #${pr.number} head '${pr.head.ref}' does not start with '${prefix}', so release-please did not create it; nothing to check.`);
    return;
  }
  if (!pr.merged_at) {
  ...
  for (const r of failed) {
    ...
    gh([
      ...
      // The provenance line is the RUN URL, not a workflow path. A hardcoded
        // `.github/workflows/release-pr-guard.yml` was right in exactly one
        // repository and would be a dead reference in every consumer that names
        // its workflow anything else -- and the run URL is what a reader
        // actually needs: the log that made this decision.
